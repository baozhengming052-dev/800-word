...
struct SyncPacket {
    ...
    private struct Header: Codable { let version: Int; let id: UUID; let kind: Kind; let digest: String?; let reason: String?; let libraryFingerprint: String? }

    // Raw payload follows a bounded JSON header; no base64 expansion of a large backup.
    func encode() throws -> Data {
        try validate()
        let header = try JSONEncoder().encode(Header(version: 1, id: id, kind: kind, digest: digest, reason: reason, libraryFingerprint: libraryFingerprint))
    ...
    static func decode(_ data: Data) throws -> SyncPacket {
        ...
        guard header.version == 1 else { throw SyncError.invalid("同步协议不一致，请更新两台设备的 App。") }
