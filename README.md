...
通过 Git Tag 管理版本，例如 `v1.0.0` 表示初始版本，`v1.1.0` 表示后续新增功能的版本。本次新增近义词补充版使用 **v2.4.3**；不要复用已失败的 `v2.4.0` 或 `v2.4.1` Tag。
...
本次建议把上面三处 `v1.1.0` 换成 `v2.4.3`，之后按实际版本递增。执行前检查待提交文件，勿混入私人备份；Tag 必须尚未存在，并指向包含全部改动的提交。无需手动改 `Info.plist` 或工程版本号。
