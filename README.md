...
**我的题目** 可完整预览、编辑、归档与恢复。题目按版本计分：修改正确答案、解析或关联词后，旧答题记录继续显示和使用原版本。新练习只选当前未归档版本，且至少关联一个未归档词；练习提交前不会显示解析、词义、笔记或手动来源备注。表单取消有未保存修改时会确认放弃；保存失败或版本过期会保留草稿并提示原因。
...
通过 Git Tag 管理版本，例如 `v1.0.0` 表示初始版本，`v1.1.0` 表示后续新增功能的版本。本次手动词题流程修复建议使用 **v2.3.2**；不要复用已有 Tag。
...
本次建议把上面三处 `v1.1.0` 换成 `v2.3.2`，之后按实际版本递增。执行前检查待提交文件，勿混入私人备份；Tag 必须尚未存在，并指向包含全部改动的提交。无需手动改 `Info.plist` 或工程版本号。
...
例如 `v2.3.2` 页面显示 `政名政利公考800词-v2.3.2.ipa`，实际下载文件为 `ZhengMingZhengLiGongKao800-v2.3.2.ipa`。中文显示名使用 GitHub 的附件 label；实际文件用拼音避免中文文件名被平台改写，校验文件内也使用真实下载文件名，不影响 TrollStore 安装。此规则对包含本次改动的新版本生效，不自动改名已经发布的附件。
